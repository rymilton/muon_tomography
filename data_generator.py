...
def main():
    ...
    data_without_object.to_csv(
        os.path.join(flags.output_directory, "theta_phi_without_object.csv"),
        index=False,
    )
    ...
    data_with_object.to_csv(
        os.path.join(flags.output_directory, "theta_phi_with_object.csv"), index=False
    )

    random_object = generate_random_object(resolution=flags.resolution)
    np.savez(
        os.path.join(flags.output_directory, "object_densities.npz"), random_object
    )
